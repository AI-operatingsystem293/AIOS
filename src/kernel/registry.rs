use crate::{
    capability::{provider::Provider, registry::ProviderRegistry},
    sdk::{agent::Agent, request::AgentRequest},
};
...
impl AgentRegistry {
    ...
    pub fn keep_library(&mut self, library: libloading::Library) {
    ...
    pub fn register(&mut self, agent: Box<dyn Agent>) {
        ...
        for capability in &manifest.capabilities {
            self.providers.register(
                Provider::new(&manifest.name, capability)
                    .describe(&manifest.description)
                    .keywords(manifest.keywords.clone()),
            );
        }

        println!("✓ Registered agent: {}", manifest.name);

        self.agents.push(agent);
    }

    pub fn start(&mut self) {
        use crate::agents::{echo::EchoAgent, math::MathAgent};
    ...
    pub fn execute_command(&mut self, command: &str, input: &str) -> Option<String> {
        for agent in self.agents.iter_mut() {
            ...
            let request = AgentRequest::new(0, 0, command, input);
            let response = agent.execute(request);

            if response.success {
                self.providers.mark_success(&manifest.name);
            } else {
                self.providers.mark_failure(&manifest.name);
            }
    ...
    pub fn find_agent_name(&self, capability: &str) -> Option<String> {
        self.providers
